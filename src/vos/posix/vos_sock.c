    ...
    const VOS_SOCK_OPT_T    *pOptions)
{
    ...
    /* enlarge send and receive buffers for UDP to TRDP_SOCKBUF_SIZE */
    {
        int         optval      = 0;
    ...
    const VOS_SOCK_OPT_T    *pOptions)
{
    ...
    /* enlarge send and receive buffers for TCP to TRDP_SOCKBUF_SIZE */
    {
        ...
        if (optval < TRDP_SOCKBUF_SIZE)
        {
            ...
            {
                (void)getsockopt(sock, SOL_SOCKET, SO_SNDBUF, (int *)&optval, &option_len);
                vos_printLog(VOS_LOG_WARNING, "vos_sockOpenTCP: send message size out of limit (max: %u)\n", optval);
                return VOS_SOCK_ERR;
            }
        }
        vos_printLog(VOS_LOG_INFO, "vos_sockOpenTCP: send message limit = %u\n", optval); 
        ...
        {        
            ...
            {
                (void)getsockopt(sock, SOL_SOCKET, SO_RCVBUF, (int *)&optval, &option_len);
                vos_printLog(VOS_LOG_WARNING, "vos_sockOpenTCP: recv message size out of limit (max: %u)\n", optval);
                return VOS_SOCK_ERR;
            }
        }
        vos_printLog(VOS_LOG_INFO, "vos_sockOpenTCP: recv message limit = %u\n", optval);
    }
