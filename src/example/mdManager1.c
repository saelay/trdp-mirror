		...
		case TRDP_MSG_MR:
			{
				...
				// LOG
				l2fLog("queue_procricz(): MD Request Received");
...
static int test_receiver()
{
	...
	// LOG
	sprintf(trdpLogString, "tlm_addListener() lisHandle = x%p",lisHandle);
	l2fLog(trdpLogString);
...
{
	fprintf(stderr,"usage: %s [--uuid] [--notify] [--request] [--reqconf] [--listener] [--dest a.b.c.d] [--l2f]\n",argv[0]);
}
