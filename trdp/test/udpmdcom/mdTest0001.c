 ...
 *		Version 0.0: s.pachera (FAR). First issues, derived from mdManager.c example application.
 *
...
static void queue_procricz()
{
    ...
    if (x_testmode == 1)
    {
        ...
        case 3002:
        {
            ...
            if (msg.Msg.msgType == TRDP_MSG_MQ)
            {
                if (msg.Msg.resultCode == TRDP_APP_TIMEOUT_ERR)
                {
...
static int test_initialize()
{
    ...
    md_config.sendParam.ttl = TRDP_MD_DEFAULT_QOS;
    md_config.flags         = 0
                              | TRDP_FLAGS_NONE * 0
