    ...
    INT32           sock)
{
	...
	vos_printf(VOS_LOG_ERROR, "Received MD packet (space: %d len: %d)\n", appHandle->pMDRcvEle->grossSize, appHandle->pMDRcvEle->dataSize);
	...
	
	{
	    ...
	    printf("sessionID       = "); for(i=0;i<16;i++) { printf("%02X ",pH->sessionID[i]); } printf("\n");
	    printf("replyTimeout    = %d\n", vos_ntohl(pH->replyTimeout   ));
	    printf("sourceURI       = "); for(i=0;i<32;i++) { if (pH->sourceURI[i]) printf("%c",pH->sourceURI[i]); } printf("\n");
	    printf("destinationURI  = "); for(i=0;i<32;i++) { if (pH->destinationURI[i]) printf("%c",pH->destinationURI[i]); } printf("\n");
	}
	...
	return TRDP_TOPO_ERR;
		...
		for(iterMD = appHandle->pMDRcvQueue; iterMD != NULL; iterMD = iterMD->pNext)
		{
			...
			if (l_comId != iterMD->u.listener.comId) continue;
					...
					case TRDP_MSG_ME: /* reply error */
						{
							...
							{
								/* check for comID .... */
								if (sender_ele->u.caller.comId != l_comId) continue;
								
								/* check for session ... */
								if (0 != memcmp(&sender_ele->sessionID,pH->sessionID,16)) continue;
