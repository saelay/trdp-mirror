...
BOOL marshallingFlag = TRUE;						/* Marshalling Enable : TURE, Marshalling Disable : FALSE */
...
int main (int argc, char *argv[])
{
	...
	while((option = getopt_long(argc, argv,
									"t:1:2:3:4:p:m:c:C:a:b:A:B:o:O:d:e:T:r:h",
									long_options,
									&option_index)) != -1){
		switch(option)
		{
		...
		break;
			...
			case '?':
				...
				printf("Usage: COMMAND [-t] [-1] [-2] [-3] [-4] [-p] [-m] [-c] [-C] [-a] [-b] [-A] [-B] [-o] [-O] [-d] [-e] [-T] [-r] [-h] \n");
...
*/
	    		if (marshallingFlag == TRUE)
	    		{
	    			...
	    			err = tau_unmarshall (pRefConMarshallDataset, PD_COMID1, (UINT8 *)((int)pTrafficStoreAddr + OFFSET_ADDRESS3), (UINT8 *) &getDataSet1, &dataSet1Size);
	    			vos_printf(VOS_LOG_DBG, "Get Traffic Store PD DATASET%d\n", DATASET_NO_1);
	    			if (err != TRDP_NO_ERR)
	    			{
	    			...
	    			err = tau_unmarshall (pRefConMarshallDataset, PD_COMID2, (UINT8 *)((int)pTrafficStoreAddr + OFFSET_ADDRESS4), (UINT8 *) &getDataSet2, &dataSet2Size);
	    			vos_printf(VOS_LOG_DBG, "Get Traffic Store PD DATASET%d\n", DATASET_NO_2);
	    			if (err != TRDP_NO_ERR)
	    			{
	    		        printf("tau_unmarshall PD DATASET%d returns error %d\n", DATASET_NO_2, err);
